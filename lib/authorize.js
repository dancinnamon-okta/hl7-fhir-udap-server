...
module.exports.authorizeHandler = async (requestPath, requestQuerystring, requestHeaders, resourceServerId) => {
	...
	const backendAuthorizeUrl = `https://${process.env.OAUTH_CUSTOM_DOMAIN_NAME_BACKEND}${requestPath}`
	...
	if(idpUri && scopes && scopes.split(" ").includes("udap") && rsConfig.role == 'dataholder') {
		...
		const client = new udapClient(rsConfig.identity.identity_store, rsConfig.identity.identity_store_pwd, rsConfig.trust_anchor, null, idpUri, process.env.ORGANIZATION_ID, process.env.ORGANIZATION_NAME, process.env.PURPOSE_OF_USE)
		
		try {
			await client.getAndValidateUdapMetadata(`${idpUri}/.well-known/udap?community=${rsConfig.community_id}`)
		}
		catch(error) {
			console.log("Unable to retrieve and/or validate community specific metadata- fetching default community metadata.")
			try {
		...
		try {
			const oauthIDPId = await oauthPlatform.getIdpIdByUri(`${rsConfig.community_id}:${idpUri}`, oauthPlatformManagementClient)
			var newIDPInfo = null
			...
			if(!oauthIDPId) {
				...
				const oauthIDPInfo = await registerOAuthIDP(`${rsConfig.community_id}:${idpUri}`, client, oidcMetadata, rsConfig, oauthPlatform, oauthPlatformManagementClient)
				newIDPInfo = oauthIDPInfo.newIdpMapping
				newIDPInfo.idp_base_url = idpUri
...
async function registerOAuthIDP(idpUri, udapClient, oidcMetadata, resourceServerConfig, oauthPlatform, oauthPlatformManagementClient) {
	...
	const authzCodeRegistrationObject = {
	  ...
	  san: resourceServerConfig.identity.san
	}
