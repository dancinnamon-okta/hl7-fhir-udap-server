...
//This is an implementation of UDAP's Trusted Dynamic Client Registration.   It is a proxy to an OAuth Dynamic Client Registartion implementation.
...
module.exports.validateUdapCommonRegistrationRequest = async (clientRegisterRequestBody, registrationRequestPath, resourceServerId) => {
  ...
  const expectedAudience = `https://${process.env.BASE_DOMAIN}${registrationRequestPath}`
  ...
  try {
    const trustAnchorObject = udapCommon.parseTrustAnchorPEM(rsConfig.trust_anchor)
    const softwareStatementDetail = await validateUdapCommonSoftwareStatement(inboundSoftwareStatement, trustAnchorObject, expectedAudience)
...
async function validateUdapCommonSoftwareStatement(inboundSoftwareStatement, trustAnchorObject, expectedAudience) {
  ...
  */
  ...
  const firstSAN = validateUdapCommonSignedSoftwareBody(jwtDetails, expectedAudience)
...
function validateUdapCommonSignedSoftwareBody(jwtDetails, expectedAudience) {
  var error = new Error()
  ...
  if (ssJwtBody.aud == "" || ssJwtBody.aud != expectedAudience)
  {
  ...
  var d = new Date()
  console.debug("IAT: " + (ssJwtBody.iat * 1000) + " Exp: " + (ssJwtBody.exp * 1000) + " Current Date: " + d.getTime())
...
module.exports.validateClientRegistrationMetaData = async (jwtDetails, editMode, resourceServerId) => {
  console.log('Checking client metadata')
  var error = new Error()
  error.code = 'invalid_client_metadata'
  const ssJwtBody = jwtDetails.body
  const rsConfig = ResourceServerConfig.getResourceServerConfig(resourceServerId)
  const metadata = rsConfig.role == 'dataholder' ? rsConfig.fhir_metadata : wellKnown.getUDAPConfiguration(resourceServerId).body

  console.log(`Validating the request in ${rsConfig.role} mode.`)
  ...
  if(!metadata.udap_versions_supported) {
    error.message = 'This resource server does not have valid metadata configured. Please update your resource servers configuration file.'
    console.error(error)
    throw error
  }
