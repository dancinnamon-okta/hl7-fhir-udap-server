...
module.exports.validateClientRegistrationMetaData = async (jwtDetails, editMode, resourceServerId) => {
  ...
  const rsConfig = ResourceServerConfig.getResourceServerConfig(resourceServerId)
  const metadata = rsConfig.role == 'dataholder' ? await wellKnown.getFHIRServerWellKnown(rsConfig.community_id) : wellKnown.getUDAPConfiguration(resourceServerId).body
  ...
  if(!metadata.udap_versions_supported) {
    error.message = 'This server does not have a valid udap metadata file.'
    console.error(error)
    throw error
  }
